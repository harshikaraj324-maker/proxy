...
export async function onRequest(context) {
  ...
  /* debug — hit /debug to see what path CF receives */
  if (path === "/debug") {
  ...
  /* relay log */
  if (path === "/api/relay-log") {
    return json(trafficLog);
  }

  /* relay SSE stream — Pages Functions flush immediately, client reconnects */
  ...
  /* ── PROXY RELAY ──
     Accepts BOTH:
       /api/relay/heartbeat          (if Android uses full prefix)
       /heartbeat  /login  etc.      (if Android hits root paths directly)
  */
  const PROXY_TARGET = (env && env.PROXY_TARGET) || "https://mr-robot-5s3.pages.dev/api";

  let suffix;
  if (path.startsWith("/api/relay/") || path === "/api/relay") {
    suffix = path.replace(/^\/api\/relay/, "") || "/";
  } else if (!path.startsWith("/api/")) {
    /* treat every non-/api path as a direct relay target */
    suffix = path;
  } else {
